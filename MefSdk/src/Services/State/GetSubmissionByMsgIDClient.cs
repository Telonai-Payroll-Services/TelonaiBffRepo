namespace MeF.Client.Services.StateServices
{
    ...
    ///<summary>
    ///  This class is the service client for invoking the GetSubmissionsByMsgID service.
    ///</summary>
    public class GetSubmissionsByMsgIDClient : MeFClientBase
    {
        #region Constructors and Destructors

        /// <summary>
        ///   Initializes a new instance of the <see cref = "GetSubmissionsByMsgIDClient" /> class.
        /// </summary>
        /// <param name = "folderName">Name of the folder.</param>
        public GetSubmissionsByMsgIDClient(string folderName)
        {
            MeFLog.LogInfo(string.Format("GetSubmissionsByMsgIDClient.ctor with folder: {0}", folderName));
        ...
        public GetSubmissionsByMsgIDClient(DirectoryInfo directory)
        {
            MeFLog.LogInfo(string.Format("GetSubmissionsByMsgIDClient.ctor with directory: {0}", directory.FullName));
            this.zipFolderLocation = directory.FullName;
        }

        /// <summary>
        ///   Initializes a new instance of the <see cref = "GetSubmissionsByMsgIDClient" /> class for in memory processing.
        /// </summary>        
        public GetSubmissionsByMsgIDClient()
        {
            MeFLog.LogInfo(string.Format("GetSubmissionsByMsgIDClient.ctor for in memory processing"));
        ...
        /// <returns>A GetSubmissionsByMsgIDResult, see <see cref = "GetSubmissionsByMsgIDResult" />.</returns>
        /// <exception cref = "ToolkitException"></exception>
        /// <exception cref = "ServiceException"></exception>
        [MethodImpl(MethodImplOptions.Synchronized)]
        public GetSubmissionsByMsgIDResult Invoke(ServiceContext serviceContext, string messageId)
        {
            MeFLog.LogInfo("GetSubmissionsByMsgIDClient.Invoke()");
            RequestHelper.TestCallPrerequisites(serviceContext);
            Validate.IsMessageIDValid(messageId, "MessageID", "");

            var proxy = new WCFGetSubmissionsByMsgIDClient();
            var responseData = new GetSubmissionsByMsgIDResponseType();
            byte[] bytes;
         
            try
            {
                var mefheader = RequestHelper.BuildRequestHeader("GetSubmissionsByMsgID", serviceContext);
                ...
                using (OperationContextScope Scope = new OperationContextScope(proxy.InnerChannel))
                {
                    ...
                    var requestData = new GetSubmissionsByMsgIDRequestType { MessageId = messageId };
                    responseData = proxy.GetSubmissionsByMsgID(ref mefheader, requestData);
                ...
                var result = new GetSubmissionsByMsgIDResult();
        ...
        private StateSubmissionContainer GetStateSubmissionContainer(
            string outerZip, GetSubmissionsByMsgIDResponseType responseData)
        {
        ...
        private StateSubmissionContainer GetStateSubmissionContainer(
            Dictionary<string, byte[]> innerzipsmap, GetSubmissionsByMsgIDResponseType responseData)
        {
